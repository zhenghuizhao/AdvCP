...
def cam_to_label_advcp(cam, cls_label, img_box=None, ignore_mid=True, cfg=None):
    b, c, h, w = cam.shape
    cls_label_rep = cls_label.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, h, w)
    valid_cam = cls_label_rep * cam

    cam_value, _pseudo_label_orig = cam.max(dim=1, keepdim=False)
    _pseudo_label_orig += 1
    _pseudo_label_orig[cam_value <= cfg.cam.bkg_score] = 0

    cam_value_valid, _pseudo_label = valid_cam.max(dim=1, keepdim=False)
    _pseudo_label += 1
    _pseudo_label[cam_value_valid <= cfg.cam.bkg_score] = 0

    # 保留 cam_orig 中不为零的数
    diff_cam_label = F.relu(cam_value) != F.relu(cam_value_valid)
    diff_label = _pseudo_label_orig != _pseudo_label

    condition = (F.relu(cam_value) != F.relu(cam_value_valid))
    diff_cam = F.relu(cam_value) * condition



    if img_box is None:
        return _pseudo_label  # Return xor_label additionally
    ...
    return valid_cam, pseudo_label, diff_cam_label, diff_label, diff_cam  # Return xor_label additionally
